...
export default function Ailments(props) {
    ...
    return (
        <div className={props.hidden ? "ailments flex" : "ailments flex color-clicked"}>
            <table className={props.hidden ? "hide-table" : "view-table"}>
                <tbody>
                    {props.monsterInfo.relationships.ailments ? props.monsterInfo.relationships.ailments.data.map((elm, index) => {
                        let type, icon;
